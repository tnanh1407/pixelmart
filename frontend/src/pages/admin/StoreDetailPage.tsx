...
import { ArrowLeft, Calendar, User, Phone, Mail, Star, ShieldCheck, Edit } from 'lucide-react'
import { storeService } from '@/services/store.service'
import { useAdminStoreMutations } from '@/hooks/admin/stores'
import { StatusBadge, LoadingState, DetailCard, DetailField, DetailInfoRow } from '@/components/admin/shared'
...
export default function StoreDetailPage() {
  ...
  const { updateStoreMutation } = useAdminStoreMutations({ onSaved: () => setShowModal(false) })

  const openEdit = () => {
    if (!store) return
    setForm({
      name: store.name || '', logo: store.logo || '', description: store.description || '',
      phone: store.phone || '', email: store.email || '',
      address: { street: store.address?.street || '', ward: store.address?.ward || '', district: store.address?.district || '', city: store.address?.city || '' },
      isVerified: store.isVerified || false, isActive: store.isActive !== undefined ? store.isActive : true,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateStoreMutation.mutate({ id: id || '', payload: { ...form, ownerId: cleanOwnerId } })
  }
  ...
  if (error || !store) {
    return (
      <div className="text-center py-20 bg-white rounded-xl border border-gray-100 shadow-sm">
        ...
        <button onClick={() => navigate('/admin/stores')} className="mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-hover transition-colors cursor-pointer">
          Quay lại danh sách
        </button>
      </div>
    )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/admin/stores')} className="p-2 bg-white hover:bg-gray-50 rounded-lg border border-gray-200 shadow-sm text-text-muted hover:text-text transition-colors cursor-pointer">
            <ArrowLeft size={18} />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-text">{store.name}</h1>
              <StatusBadge active={store.isActive} activeLabel="Đang hoạt động" />
            </div>
            <p className="text-sm text-text-muted mt-1">ID: {store._id}</p>
          </div>
        </div>
        <button onClick={openEdit} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg font-medium hover:bg-primary-hover transition-colors shadow-sm text-sm cursor-pointer">
          <Edit size={16} /> Chỉnh sửa cửa hàng
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden w-full">
        <div className="p-6 space-y-6">
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden border border-gray-200 shrink-0">
              {store.logo ? (
                ...
                <div className="w-full h-full flex items-center justify-center"><ShieldCheck size={32} className="text-text-muted" /></div>
              )}
            </div>
            <div>
              <h4 className="text-xl font-bold text-text flex items-center gap-2">
                ...
                {store.isVerified && (
                  <Badge className="bg-blue-500 hover:bg-blue-600 text-white flex items-center gap-1 py-0.5 px-2">
                    <ShieldCheck size={12} className="fill-white text-blue-500" /> Đã xác minh
                  </Badge>
                )}
              </h4>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-3 text-sm text-text bg-gray-50 p-4 rounded-lg border border-gray-100">
              <div className="flex items-center gap-2"><Phone size={16} className="text-text-muted" /><span>{store.phone || 'Chưa cập nhật số điện thoại'}</span></div>
              <div className="flex items-center gap-2"><Mail size={16} className="text-text-muted" /><span className="truncate">{store.email || 'Chưa cập nhật email'}</span></div>
              <div className="flex items-center gap-2"><User size={16} className="text-text-muted" />
                <span className="text-xs text-text-muted font-mono">
                  Chủ sở hữu: {typeof store.ownerId === 'object' && store.ownerId !== null ? `${store.ownerId.name} (${store.ownerId.email})` : store.ownerId}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
              ...
              <div className="flex items-center justify-center gap-1 mt-1 text-base font-bold text-amber-600">
                <Star size={14} className="fill-amber-500 text-amber-500" />{store.ratingsAverage.toFixed(1)}
              </div>
            </div>
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
          ...
          <div className="border-t border-gray-100 pt-6 grid grid-cols-2 gap-6 text-sm text-text-muted">
            <div>
              ...
              <StatusBadge active={store.isActive} activeLabel="Hoạt động" inactiveLabel="Đang khóa" />
            </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
          <button onClick={() => navigate('/admin/stores')} className="px-4 py-2 text-sm font-medium text-text bg-white border border-gray-200 hover:bg-gray-50 rounded-lg transition-colors shadow-sm cursor-pointer">
            Quay lại
          </button>
        </div>
      </div>
