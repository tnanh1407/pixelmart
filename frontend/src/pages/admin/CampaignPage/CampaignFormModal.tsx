...
export function useCampaignForm() {
  ...
  const populateForm = useCallback((banner: ICampaign) => {
    setForm({
      title: banner.title || '',
...
function BasicTab({ form, setForm, isUploading, handleFileChange }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Hình ảnh chiến dịch *</label>
        <div className="space-y-2">
          <div className="flex gap-3">
            ...
            <label className={`flex items-center gap-2 px-4 py-2 border border-dashed rounded-lg text-sm font-medium cursor-pointer transition-colors whitespace-nowrap ${
              isUploading ? 'border-gray-300 bg-gray-50 text-gray-400 cursor-not-allowed' : 'border-indigo-300 hover:border-indigo-500 hover:bg-indigo-50/50 text-indigo-600'
            }`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          {!!form.durationInDays && <p className="text-[10px] text-gray-400 mt-0.5">Vô hiệu hóa do đã nhập số ngày hiển thị</p>}
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })}
            className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" min={0} />
        </div>
        <div>
          ...
          {(!!form.startDate || !!form.endDate) && <p className="text-[10px] text-gray-400 mt-0.5">Vô hiệu hóa do đã nhập ngày bắt đầu/kết thúc</p>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <textarea value={form.sapo} onChange={(e) => setForm({ ...form, sapo: e.target.value })} rows={2}
          className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Danh sách điểm nổi bật (Highlights - Mỗi dòng là một điểm)</label>
    ...
    <div className="space-y-4">
      ...
      {contentSections.length === 0 ? (
        <div className="text-center py-10 border-2 border-dashed border-gray-100 rounded-xl bg-gray-50/50">
          ...
          <button type="button" onClick={addSection} className="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:underline">
          ...
          {contentSections.map((section, index) => (
            <div key={index} className="border border-gray-200 rounded-xl p-4 bg-gray-50/30 space-y-3 relative group">
              <button type="button" onClick={() => removeSection(index)}
                className="absolute top-3 right-3 p-1.5 text-gray-400 hover:text-red-500 hover:bg-white rounded-lg border border-transparent hover:border-gray-100 transition-colors shadow-sm" title="Xóa phần này">
                <Trash2 size={14} />
              </button>
              <div className="text-xs font-bold text-gray-400 uppercase tracking-wider">Phần #{index + 1}</div>
              <div>
                ...
                <input type="text" value={section.title ?? ''} onChange={(e) => updateSection(index, 'title', e.target.value)}
                  className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Tiêu đề phụ..." />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Nội dung phần</label>
                <textarea value={section.content ?? ''} onChange={(e) => updateSection(index, 'content', e.target.value)} rows={3}
                  className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Nội dung chi tiết..." />
              </div>
            </div>
          ))}
        </div>
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col overflow-hidden p-0 gap-0" showCloseButton={false}>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col">
          ...
          <DialogFooter className="px-6 py-4 border-t border-gray-100 bg-white -mx-0 -mb-0 rounded-none border-x-0">
            <button type="button" onClick={closeModal} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
              Hủy
            </button>
