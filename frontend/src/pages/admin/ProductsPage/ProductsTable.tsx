  ...
  if (products.length === 0) {
    return (
      <div className="py-20 text-center">
        <Package size={48} className="mx-auto mb-3 text-muted-foreground/50" />
      ...
      <TableBody>
        {products.map((product) => (
          <TableRow key={product._id}>
            <TableCell className="px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="max-w-[220px] truncate text-sm font-medium text-foreground">{product.name}</p>
                  <p className="text-xs text-muted-foreground">{product.brand || 'Chưa có thương hiệu'}</p>
                </div>
              </div>
            </TableCell>
            <TableCell className="px-6 py-4">
            ...
            <TableCell className="px-6 py-4">
              <span className={product.stock <= 0 ? 'text-sm font-medium text-destructive' : 'text-sm font-medium text-foreground'}>
                {product.stock}
              </span>
            </TableCell>
            <TableCell className="px-6 py-4">
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm text-foreground">{product.ratingsAverage?.toFixed(1) || '0.0'}</span>
                <span className="text-xs text-muted-foreground">({product.ratingsQuantity || 0})</span>
              </div>
            </TableCell>
            <TableCell className="px-6 py-4">
            ...
            <TableCell className="px-6 py-4 text-right">
              <div className="flex items-center justify-end gap-1">
                <Button
                  ...
                  className={product.isActive ? 'text-green-600 hover:bg-green-50 hover:text-green-700' : 'text-muted-foreground hover:bg-muted'}
                ...
                <Button
                  ...
                  className={product.isFeatured ? 'text-amber-600 hover:bg-amber-50 hover:text-amber-700' : 'text-muted-foreground hover:bg-muted'}
