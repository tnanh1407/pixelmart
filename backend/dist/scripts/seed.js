...
async function processSeedImages(name, data, options) {
    if (SKIP_CLOUDINARY || data.length === 0)
        return data;
...
async function clearCloudinary() {
    ...
    for (const prefix of prefixes) {
        try {
            ...
            totalDeleted += count;
            if (count > 0)
                console.log(`  ${prefix}: deleted ${count} images`);
        }
        catch (err) {
...
async function seed() {
    ...
    for (let i = 0; i < userSeedData.length; i++) {
        ...
        if (!SKIP_CLOUDINARY && data.avatar) {
            ...
            if (avatarUrl)
                process.stdout.write(`  ${i + 1}/${userSeedData.length} ${data.name}\r`);
        }
        let hashedPassword;
        if (data.password)
    ...
    insertedUsers.forEach((u) => { if (u.role in roleCounts)
        roleCounts[u.role]++; });
    ...
    const catData = await processSeedImages("Categories", [...categorySeedData], {
        stringFields: ["image"],
        idField: "_id",
        folderFn: (item) => `pixelmart/categories/${item._id}`,
    });
    ...
    const prodData = await processSeedImages("Products", [...productSeedData], {
        ...
        arrayFields: ["images"],
        idField: "_id",
        folderFn: (item) => `pixelmart/products/${item._id}`,
    });
