...
class ProductService {
    async getProducts(query = {}) {
        const { page = 1, limit = 12, search, categoryId, storeId, minPrice, maxPrice, flashSaleActive, sort } = query;
    ...
    async createProduct(userId, userRole, data) {
    ...
    async updateProduct(userId, userRole, id, data) {
    ...
    async deleteProduct(userId, userRole, id) {
        const product = await this.getProductById(id);
    ...
    async uploadProductImage(file, productId, userId, userRole) {
    ...
    async uploadProductImages(files, productId, userId, userRole) {
        const product = await this.getProductById(productId);
        const urls = [];
    ...
    async removeProductImage(productId, imageUrl, userId, userRole) {
        const product = await this.getProductById(productId);
...
async function getActiveFlashSaleIds(now) {
