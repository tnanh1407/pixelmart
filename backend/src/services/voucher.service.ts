import Voucher, { IVoucher } from "../models/voucher.model.js";
...
class VoucherService {
  ...
  async getAvailableVouchers(storeId?: string, orderValue?: number) {
    const now = new Date();
    const filter: any = {
      isActive: true,
      status: "active",
  ...
  async validateVoucher(code: string, orderValue: number, storeId?: string) {
    ...
    if (orderValue < voucher.minOrderValue) {
      throw new AppError(
        `Giá trị đơn hàng tối thiểu để áp dụng voucher là ${voucher.minOrderValue.toLocaleString("vi-VN")}đ`,
        400
      );
    }
  ...
  async createVoucher(data: Partial<IVoucher>) {
    const { code } = data;
    if (!code) {
  ...
  async updateVoucher(id: string, data: Partial<IVoucher>) {
    const voucher = await this.getVoucherById(id);

    if (data.name !== undefined) voucher.name = data.name;
    if (data.description !== undefined) voucher.description = data.description;
    if (data.value !== undefined) voucher.value = data.value;
    if (data.minOrderValue !== undefined) voucher.minOrderValue = data.minOrderValue;
    if (data.maxDiscount !== undefined) voucher.maxDiscount = data.maxDiscount;
    if (data.usageLimit !== undefined) voucher.usageLimit = data.usageLimit;
    if (data.startDate !== undefined) voucher.startDate = data.startDate;
    if (data.endDate !== undefined) voucher.endDate = data.endDate;
    if (data.isActive !== undefined) voucher.isActive = data.isActive;
