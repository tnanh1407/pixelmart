...
import { SEPAY_CONFIG, SepayWebhookPayload } from "../config/sepay.js";
...
class SepayService {
  ...
  async createPayment(orderId: string): Promise<{ qrCodeUrl: string }> {
  ...
  async processWebhook(payload: SepayWebhookPayload): Promise<{ success: boolean }> {
  ...
  private parseOrderCode(content: string): string | null {
